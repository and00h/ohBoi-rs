...
use log::debug;
use crate::cpu::{CpuFlag, Register16, Register8};
use crate::cpu::Cpu;
use Register16::*;
use Register8::*;
...
fn unknown(cpu: &mut Cpu, opcode: u8) {
    panic!("Unknown opcode! 0x{:#04X} PC: 0x{:#04X}", opcode, cpu.pc);
}
...
impl InstArg {
    pub fn word(&self) -> u16 {
        match self {
            ...
            Self::None => panic!("InstArg::word() called on a None value!")
        }
    }
...
fn nop(cpu: &mut Cpu) { cpu.state = CpuState::FinishedExecution }
